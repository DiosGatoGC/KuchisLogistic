    ...
    <div className="users-page">
      ...
      <div className="users-layout">
        <Surface className="user-create-card">
          ...
          <label className="field"><span className="field__label">Rol</span><select className="input" value={createForm.role} disabled={busyKey === "create" || blockedKeys.has("create")} onChange={(event) => setCreateForm((current) => ({ ...current, role: event.target.value as UserRole }))}>{USER_ROLES.map((role) => <option key={role} value={role}>{MANAGED_USER_ROLE_LABELS[role]}</option>)}</select>{createErrors.role && <span className="field__error" role="alert">{createErrors.role}</span>}</label>
